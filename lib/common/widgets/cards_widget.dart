...
import 'package:petcross/common/utils/coloors.dart';
import 'package:petcross/src/presentaion/controllers/match_screen_controller.dart';
import 'package:petcross/src/presentaion/controllers/tabs/cards_tab_controller.dart';
import 'package:petcross/src/presentaion/controllers/tabs/chats_tab_controller.dart';
            ...
            child: CardSwiper(
                ...
                onUndo: (previousIndex, currentIndex, direction) {
                  if (widget.matchScreen) {
                    ...
                    provider.addToPending(data.cards[currentIndex].user.uid);
                  } else {
                    provider
                        .removeFromBlocked(data!.cards[currentIndex].user.uid);
                    provider.removeFromLiked(data.cards[currentIndex].user.uid);
                  }
                ...
                onSwipe: (previousIndex, currentIndex,
                  ...
                  if (direction.name == "left") {
                    if (widget.matchScreen) {
                      provider.deletePendingAndBlock(
                          data!.cards[previousIndex].user.uid);
                      provider.setIndex(currentIndex ?? data.cards.length - 1);
                    } else {
                      provider
                    ...
                    if (widget.matchScreen) {
                      provider.deletePendingAndLike(
                          data!.cards[previousIndex].user.uid);
                      provider.setIndex(currentIndex ?? data.cards.length - 1);
                      sendTextMessage(
                          context, data.cards[previousIndex].user.uid);
                      await MessagingApi().callOnFcmApiSendPushNotifications(
                          data.cards[previousIndex].user.fcmToken,
                          "mutual_match".tr(),
                          "mutual_match_body".tr());
                    } else {
                      provider.addToLiked(data!.cards[previousIndex].user.uid);
                      provider.setIndex(currentIndex ?? data.cards.length - 1);
                      await MessagingApi().callOnFcmApiSendPushNotifications(
                          data.cards[previousIndex].user.fcmToken,
                          "new_match".tr(),
